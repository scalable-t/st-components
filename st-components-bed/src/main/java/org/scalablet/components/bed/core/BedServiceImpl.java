...
public class BedServiceImpl implements BedFacadeService {
    ...
    /**
     * Construct with bedExecutorRegistry
     *
     * @param bedExecutorRegistry registry
     * @param serializer          serializer
     * @param repository          repository stores tasks
     * @param runner              Task runner
     * @param configuration       configuration
     * @param tracer              tracer support
     */
    ...
    public <T extends BedExecutorCmd> void submit(Class<? extends BedExecutor<T>> executorType, T cmd) {
        ...
        // get executor
        final BedExecutor<T> executor = bedExecutorRegistry.getExecutor(executorType);

        // create task and save
        final BedExecutor.RetryControl retryControl;
        retryControl = getRetryControl(cmd, executorTypeName, executor);
        ...
        repository.save(bedTask);
        // Tell dispatcher a task has been created
        dispatcher.taskCreated(bedTask);
